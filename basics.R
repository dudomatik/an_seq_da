...
# Declare variables of different types, types are inferred automaticaly
my_numeric = 42
...
#str() function allows one to investigate R objects
str(my_numeric)


#you can also change type
my_numericCharacter = as.character(my_numeric)
str(my_numeric)
str(my_numericCharacter)
...
#setting the name of columns, extremely useful for orientation in large matrices/ datasets
...
#Another important type in R: Lists
#similar to python dictionaries but slightly different syntax
world_1 = list(money = 1000, life =  1, happiness = 0, pollution = 10000)
world_2 = list(money = 10, life =  1, happiness = 10, pollution = 10)
world_3 = list(money = 800, life =  100, happiness = 100, pollution = 50)


#lists can be accessed with the $ key
world_2$money

#lists can be nested!
your_choice = list(one=world_1,two = world_2,three = world_3)

#investigate the structure with str()
str(your_choice)

#lists don't even need explicit keys and can have mixed type

simple_list = list(1,2023, test = TRUE, myMatrix)

#access via key or via number
simple_list[[1]]
simple_list$test
...
#Another example: arrivals is a quarterly multiple time series, i.e. it contains quarterly arrivals 
